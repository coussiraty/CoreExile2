...
namespace SekhemaHelper
{
    ...
    public static class SekhemaReader
    {
        ...
        public static SekhemaFloor Read(IUiElement panel)
        {
            ...
            if (d1b0 != null)
            {
                for (int li = 0; li < floor.Layers.Count && li < d1b0.ChildCount; li++)
                {
                    var layerEl = d1b0.Child(li);
                    if (layerEl == null)
                        continue;
                    for (int ri = 0; ri < floor.Layers[li].Count && ri < layerEl.ChildCount; ri++)
                    {
                        var w = layerEl.Child(ri);
                        if (w == null)
                            continue;
        ...
        private static IUiElement FindLayersContainer(IUiElement root, int layerCount)
        {
            if (root == null || layerCount <= 0)
                return null;
            var queue = new Queue<(IUiElement el, int depth)>();
            ...
            while (queue.Count > 0)
            {
                ...
                if (el == null || depth > 5)
                    continue;
                if (el.ChildCount == layerCount)
                {
                    var first = el.Child(0);
                ...
                if (n > 0 && n <= 64)
                    for (int i = 0; i < n; i++)
                        queue.Enqueue((el.Child(i), depth + 1));
            }
            return null;
        }
