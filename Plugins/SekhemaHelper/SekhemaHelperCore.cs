namespace SekhemaHelper
{
    ...
    public sealed class SekhemaHelperCore : Plugin<SekhemaHelperSettings>
    {
        ...
        {
            // Game pid for the cross-process reader (refreshed every frame; cheap).
            Mem.Pid = Ctx.Game.Pid;
            ...
            if (Settings.DebugEnable)
            {
                foreach (var layer in floor.Layers)
                    foreach (var room in layer)
                    {
                        ...
                        drawList.AddText(room.ScreenPos, Draw.Color(Settings.TextColor), txt);
                    }
            }
            ...
            if (Settings.DrawBestPath)
            {
                ...
                uint color = Draw.Color(Settings.BestPathColor);
        ...
        private void ReadPlayerStats(out int evasion, out int energyShield, out int armour, out int life, out bool qotf)
        {
            evasion = 0;
            energyShield = 0;
            armour = 0;
            ...
            if (player == null || !player.TryGetComponent<IStats>(out var stats))
                return;
            // Confirmed live 2026-06-10 (Research --stats vs character sheet):
            //  - effective Evasion / Max ES match the items layer (NOT the sum of both layers).
            ...
            //    solo Sekhema run, so we read the items layer here (IStats.TryGetItemStat).
            //  - the Queen-of-the-Forest stat is granted via the buff/action layer, so it is read
            //    through the summed total (IStats.GetTotalStat).
            // Armour and Max Life feed the relevance-based affliction model (WeightCalculator
            // .DynamicAffliction).
            stats.TryGetItemStat(GameStat.EvasionRating, out evasion);
            stats.TryGetItemStat(GameStat.MaximumEnergyShield, out energyShield);
            stats.TryGetItemStat(GameStat.Armour, out armour);
            stats.TryGetItemStat(GameStat.MaximumLife, out life);
            qotf = stats.GetTotalStat(GameStat.MovementSpeedFromEvasion) > 0;
        }
        ...
        private void DebugHud(ImDrawListPtr drawList, string text)
        {
            ...
            drawList.AddRectFilled(pos - new Vector2(4, 2), pos + size + new Vector2(4, 2),
                Draw.Color(new Vector4(0f, 0f, 0f, 0.8f)));
            drawList.AddText(pos, Draw.Color(new Vector4(1f, 0.9f, 0.2f, 1f)), "SekhemaHelper: " + text);
        }
