namespace Atlas
{
    ...
    public sealed class Atlas : Plugin<AtlasSettings>
    {
        ...
        private struct NodeData
        {
            ...
            public List<GridPoint> ConnectedGridPositions;
        ...
        private Dictionary<GridPoint, List<GridPoint>> cachedRouteGraph;
        private HashSet<GridPoint> cachedAccessible;
        private Dictionary<GridPoint, GridPoint> cachedBfsTree;
        ...
        public override void DrawSettings()
        {
            ...
            Draw.ToolTip("Draw connected-node and badge counts under each map label on the Atlas.");
            ImGui.Checkbox("Show Content", ref Settings.ShowContent);
            Draw.ToolTip("Draw the node's content under each map label, using the known names.");
            if (Settings.ShowContent)
            {
                ImGui.Indent();
                ImGui.Checkbox("Debug Content", ref Settings.ShowContentDebug);
                Draw.ToolTip("Also show unmapped content as its raw 0x value (for identifying new content).");
                ImGui.Unindent();
            }
        ...
        public override void DrawUI()
        {
            ...
            if (!this.Ctx.Game.IsForeground && !isGameHelperForeground)
                return;

            var player = this.Ctx.Game.InGame.Player;
            if (!player.TryGetComponent<IRender>(out _))
                return;

            var drawList = ImGui.GetBackgroundDrawList();

            var atlasUi = this.Ctx.Ui.Atlas;
            if (!atlasUi.Exists || !atlasUi.IsVisible)
                return;
            ...
            var atlasCount = atlasUi.ChildCount;
            ...
            var allCenters = new Dictionary<GridPoint, Vector2>(nodeCache.Count);
            foreach (var nd in nodeCache)
            {
                var nu = atlasUi.Child(nd.Index);
                if (nu == null) continue;
            ...
            using (new FontScaleScope(uiScale))
            {
                ...
                {
                    shiftedCenters = new Dictionary<GridPoint, Vector2>(allCenters.Count);
                ...
                if (Settings.ShowAtlasGraph)
                {
                    ...
                    static bool IsCanonical(GridPoint a, GridPoint b)
                    {
                    ...
                    foreach (var nd in nodeCache)
                    {
                        ...
                        foreach (var dst in nd.ConnectedGridPositions)
                        {
                            ...
                            drawList.AddLine(sa, da, Draw.Color(Settings.AtlasGraphLineColor), lineTh);
                        }
                    }
                }
                ...
                foreach (var nd in nodeCache)
                {
                    ...
                    if (Settings.DrawLinesToTowers && towers.Contains(mapName) && !completed)
                        { routeTarget = true; routeColor = Draw.Color(Settings.TowerPathColor); maxHops = Settings.TowerMaxHops; }
                    else if (Settings.DrawLinesToSearch && doSearch
                        && searchList.Any(s => mapName.Contains(s, StringComparison.OrdinalIgnoreCase)))
                        { routeTarget = true; routeColor = Draw.Color(Settings.SearchPathColor); maxHops = Settings.SearchMaxHops; }
                    else if (Settings.DrawLinesToUniqueMaps && !completed
                        && string.Equals(nd.Type, "unique", StringComparison.OrdinalIgnoreCase))
                        { routeTarget = true; routeColor = Draw.Color(Settings.UniquePathColor); maxHops = Settings.UniqueMaxHops; }
                        ...
                        { routeTarget = true; routeColor = Draw.Color(Settings.LineagePathColor); maxHops = Settings.LineageMaxHops; }
                    else if (Settings.DrawLinesToQuests && !completed && QuestsMaps.Contains(mapName))
                        { routeTarget = true; routeColor = Draw.Color(Settings.QuestsPathColor); maxHops = Settings.QuestsMaxHops; }
                        ...
                        { routeTarget = true; routeColor = Draw.Color(Settings.ArbiterPathColor); maxHops = Settings.ArbiterMaxHops; }
                    else if (Settings.DrawLinesToAtlasProgression && !completed && AtlasProgressionMaps.Contains(mapName))
                        { routeTarget = true; routeColor = Draw.Color(Settings.AtlasProgressionPathColor); maxHops = Settings.AtlasProgressionMaxHops; }
                    else if (Settings.DrawLinesToRitual && !completed && RitualMaps.Contains(mapName))
                        { routeTarget = true; routeColor = Draw.Color(Settings.RitualPathColor); maxHops = Settings.RitualMaxHops; }
                    else if (Settings.DrawLinesToBreach && !completed && BreachMaps.Contains(mapName))
                        { routeTarget = true; routeColor = Draw.Color(Settings.BreachPathColor); maxHops = Settings.BreachMaxHops; }
                    else if (Settings.DrawLinesToExpedition && !completed && ExpeditionMaps.Contains(mapName))
                        { routeTarget = true; routeColor = Draw.Color(Settings.ExpeditionPathColor); maxHops = Settings.ExpeditionMaxHops; }
                    else if (Settings.DrawLinesToAbyss && !completed && AbyssMaps.Contains(mapName))
                        { routeTarget = true; routeColor = Draw.Color(Settings.AbyssPathColor); maxHops = Settings.AbyssMaxHops; }
                    else if (Settings.DrawLinesToTemple && !completed && TempleMaps.Contains(mapName))
                        { routeTarget = true; routeColor = Draw.Color(Settings.TemplePathColor); maxHops = Settings.TempleMaxHops; }
                    ...
                    var nodeUi = atlasUi.Child(nd.Index);
                    if (nodeUi == null)
                        continue;
                    ...
                    if (routeTarget)
                    {
                        ...
                        if (path != null && path.Count > 0 && hops <= maxHops)
                        {
                            ...
                            if (shiftedCenters.TryGetValue(path[0], out var entryC))
                            {
                                ...
                                drawList.AddCircleFilled(entryC, sr, Draw.Color(new Vector4(0.2f, 1f, 0.2f, 1f)));
                            ...
                            drawList.AddRectFilled(hp - hpad, hp + hts + hpad, Draw.Color(new Vector4(0, 0, 0, 0.75f)), 3f * uiScale);
                            drawList.AddText(hp, Draw.Color(new Vector4(1f, 0.9f, 0.2f, 1f)), ht);
                        }
                        }
                    ...
                    if (Settings.ShowBiomeBorder && Biomes.TryGetValue(nd.BiomeId, out var biome) && biome.Show)
                    {
                        ...
                        drawList.AddRect(outMin, outMax, Draw.Color(biomeColor),
                            outRounding, ImDrawFlags.RoundCornersAll, bBorderTh);
                    }

                    drawList.AddRectFilled(bgPos, bgPos + bgSize, Draw.Color(backgroundColor), rounding);
                    drawList.AddText(drawPosition, Draw.Color(fontColor), mapName);
                    ...
                    if (Settings.ShowMapCounts)
                    {
                        ...
                        drawList.AddText(countPos, Draw.Color(fontColor), countText);
        ...
        private void RefreshNodeCache(IUiElement atlasUi, int atlasCount)
        {
            nodeCache.Clear();
            foreach (var map in this.Ctx.Ui.AtlasMaps)
            {
            ...
            cachedRouteGraph = new Dictionary<GridPoint, List<GridPoint>>();
            cachedAccessible = new HashSet<GridPoint>();
            var routingExcluded = new HashSet<GridPoint>();
            foreach (var nd in nodeCache)
            {
            ...
            cachedBfsTree = MultiSourceBfs(cachedRouteGraph, cachedAccessible, new HashSet<GridPoint>(), routingExcluded);
        }
        ...
        private static Dictionary<GridPoint, GridPoint> MultiSourceBfs(
            Dictionary<GridPoint, List<GridPoint>> graph,
            HashSet<GridPoint> sources,
            HashSet<GridPoint> blocked,
            HashSet<GridPoint> noPass)
        {
        ...
        private static List<GridPoint> PathFromAccessible(
            GridPoint target,
            Dictionary<GridPoint, GridPoint> cameFrom,
            HashSet<GridPoint> sources)
        {
            if (sources.Contains(target))
                return new List<GridPoint> { target };
            if (!cameFrom.ContainsKey(target))
                return null;

            var path = new List<GridPoint> { target };
            var cur = target;
        ...
        private static void DrawNodePath(
            ...
            List<GridPoint> path,
            Dictionary<GridPoint, Vector2> centers,
            uint color,
            float thickness,
        ...
        {
            var path = Path.Join(DirectoryPath, "json", "biome.json");
            if (!File.Exists(path))
                return;
        ...
        {
            var path = Path.Join(DirectoryPath, "json", "content.json");
            if (!File.Exists(path))
                return;
            ...
            ref float nextRowTopY, float rowGap, Vector4 fontColor)
        {
            ...
            drawList.AddText(pos, Draw.Color(fontColor), text);
            ...
            ref float nextRowTopY, float rowGap, float uiScale)
        {
            ...
            for (int i = 0; i < infos.Count; i++)
            {
                ...
                drawList.AddRectFilled(squareMin, squareMax, Draw.Color(info.BgColor));

                var textSize = ImGui.CalcTextSize(abbrev);
                var textPos = squareMin + (boxSize - textSize) * 0.5f;
                drawList.AddText(textPos, Draw.Color(info.FtColor), abbrev);
        ...
        private static bool TriangleButton(string id, float buttonSize, Vector4 color, bool isUp)
        {
            ...
            drawList.AddTriangleFilled(p1, p2, p3, Draw.Color(color));
        ...
        private bool InventoryPanel()
        {
            return this.Ctx.Ui.RightPanel.IsVisible;
        }
        ...
        private static void PathRow(string label, ref bool enabled, ref Vector4 color, ref int maxHops)
        {
            ...
            Draw.ToolTip("Maximum path length in maps to clear.");
            ImGui.SameLine();
            ImGui.Text(label);
        }
