namespace RunecraftHelper
{
    ...
    public sealed partial class RunecraftHelperCore
    {
        ...
        private bool LoadMonolithData()
        {
            ...
            try
            {
                var path = Path.Join(this.DirectoryPath, "expedition2_recipes.json");
        ...
        private void DrawMonolithMapLabels()
        {
            ...
            if (largeMap == null || !largeMap.IsVisible || this.Ctx.Ui.WorldMapPanel.IsVisible) return;

            var player = this.Ctx.Game.InGame.Player;
            if (player == null || !player.TryGetComponent<IRender>(out var playerRender)) return;
        ...
        private List<MonoView> EnumerateMonoliths()
        {
            ...
            if (!this.Ctx.Game.IsInGame) return list;
            int areaLevel = this.Ctx.Game.InGame.AreaLevel;

            Vector2 pg = default;
            bool havePlayer = false;
            var localPlayer = this.Ctx.Game.InGame.Player;
            if (localPlayer != null && localPlayer.TryGetComponent<IRender>(out var ppr))
            {
            ...
            foreach (var e in this.Ctx.Entities.Awake)
            {
                ...
                if (e == null || !e.IsValid || e.State == EntityState.Useless) continue;
                var path = e.Path;
                if (string.IsNullOrEmpty(path) ||
                    path.IndexOf(MonolithDevicePath, StringComparison.OrdinalIgnoreCase) < 0)
                    continue;
                if (!e.TryGetComponent<IStateMachine>(out var sm)) continue;
        ...
        private bool TryResolveStation(IStateMachine sm, IntPtr deviceAddr, out IntPtr station, out string diag)
        {
            station = IntPtr.Zero;
            diag = string.Empty;
